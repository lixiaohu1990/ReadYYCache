...
1.存：根据threshold属性判断所存储的对象是文件存储还是数据库存储，不过即使是文件存储依旧会在数据库中添加一行数据，记录相关文件名和访问时间属性。
2.取：根据key进行数据库检索，获得其文件名，然后根据文件名判断是文件读取，还是数据库读取，假若是数据库读取，则会将所在列的所有key生成一个YYKVStorageItem，然后对这个item进行操作。
